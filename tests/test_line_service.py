...
@pytest.mark.asyncio
async def test_reply_text_sends_expected_request() -> None:
    captured: dict[str, object] = {}
    ...
    try:
        await service.reply_text(reply_token="reply-token", text="result")
        request = captured["request"]
        assert isinstance(request, httpx.Request)
        assert request.url.path == "/v2/bot/message/reply"
        assert b'"replyToken":"reply-token"' in request.content
        assert b'"text":"result"' in request.content
    finally:
        await service.close()
